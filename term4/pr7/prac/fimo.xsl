...
<!--	diagrams and annotation not supported for fimo 
            <td bgcolor="#FFDDFF">
        ...
        <xsl:choose>
          <xsl:when test="$num_qvalues > 0">
            ...
            <xsl:for-each select="document('cisml.xml')/cis:cis-element-search/cis:pattern/cis:scanned-sequence/cis:matched-element">
              ...
              <tr>
                <!-- make an Entrez link : under development
                <xsl:if test="$alphabet = 'nucleotide'">nucleotide</xsl:if>
                <span style="background-color: #DDDDFF;">
                <a href="http://www.ncbi.nlm.nih.gov/entrez/query.fcgi?db=nucleotide&cmd=Search&term={$seq_name}">E</a></span>
                -->
	        <!-- Buttons are pointless since only one section.
                <td>
